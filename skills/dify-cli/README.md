...
A command-line tool for authoring Dify DSL (workflow / chatflow) YAML files declaratively. Write a spec, validate it, `apply` generates the DSL.
...
If import fails with "client-side exception", run `dify-cli validate` first, and ensure the frontend defaults bundle matches the Dify version (see [Regenerating schemas](#regenerating-schemas-after-a-dify-upgrade) below).
