...
- `implementation_hint` (optional): natural-language description of what the node should do, including how `template_inputs` variables are used in the impl file. Only for nodes that need impl files (code/llm/http/etc). apply ignores it; the implementation sub-agent reads it. Example: `"implementation_hint": "POST 调用 API，body 用 {{#start.product#}} 作为产品参数，url 用 {{#env.API_URL#}}"`
